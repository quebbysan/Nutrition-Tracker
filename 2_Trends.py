...
score_rows = nt.get_daily_health_scores(str(start_date), str(end_date))
score_by_date = dict(score_rows)
scores = [score_by_date.get(str(d)) for d in all_dates]

fig_score = go.Figure()
fig_score.add_trace(go.Scatter(
    x=x_labels, y=scores, name="Health Score",
    line=dict(color="#4ADE80", width=3),
    connectgaps=True,
))
fig_score.update_layout(**base_layout("score (0-100)"))
